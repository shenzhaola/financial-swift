...
public class Field122 extends Field implements Serializable {
	 ...
	 */
	public Field122(final String value) {
		super(value);
	}
	 ...
	 * @since 7.8
	 */
	public Field122(final Tag tag) {
		this();
		if (tag == null) {
			throw new IllegalArgumentException("tag cannot be null.");
		}
		if (!StringUtils.equals(tag.getName(), "122")) {
			throw new IllegalArgumentException("cannot create field 122 from tag "+tag.getName()+", tagname must match the name of the field.");
		}
		parse(tag.getValue());
	}
	...
	*/
	 ...
	 * @param component1 Number with the Number of Messages content to set
	 */
   ...
   public String parserPattern() {
	 ...
	 * @return the static value of Field122.COMPONENTS_PATTERN
	 */
	@Override
	...
	/**
	 * Gets the first instance of Field122 in the given message.
	 ...
	 */ 
	public static List<Field122> getAll(final SwiftTagListBlock block) {
		if (block == null || block.isEmpty()) {
			return java.util.Collections.emptyList();
		}
		final Tag[] arr = block.getTagsByName(NAME);
		if (arr != null && arr.length > 0) {
			final List<Field122> result = new ArrayList<Field122>(arr.length);
			for (final Tag f : arr) {
	...
	/**
	 * This method deserializes the JSON data into a Field122 object.

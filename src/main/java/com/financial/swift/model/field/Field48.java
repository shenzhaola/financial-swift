...
public class Field48 extends Field implements Serializable {
	 ...
	 */
	public Field48(final String value) {
		super(value);
	}
	 ...
	 * @since 7.8
	 */
	public Field48(final Tag tag) {
		this();
		if (tag == null) {
			throw new IllegalArgumentException("tag cannot be null.");
		}
		if (!StringUtils.equals(tag.getName(), "48")) {
   ...
   public String parserPattern() {
	 ...
	 * @return the static value of Field48.COMPONENTS_PATTERN
	 */
	@Override
	...
	/**
	 * Gets the first instance of Field48 in the given message.
	...
	/**
	 * Gets a list of all occurrences of the field Field48 in the given message
	...
	/**
	 * Gets a list of all occurrences of the field Field48 from the given block
	 * an empty list is returned if none found.
	 *
	 * @param block may be empty or null in which case an empty list is returned 
	 */ 
	public static List<Field48> getAll(final SwiftTagListBlock block) {
		if (block == null || block.isEmpty()) {
			return java.util.Collections.emptyList();
		}
		final Tag[] arr = block.getTagsByName(NAME);
		if (arr != null && arr.length > 0) {
			final List<Field48> result = new ArrayList<Field48>(arr.length);
			for (final Tag f : arr) {

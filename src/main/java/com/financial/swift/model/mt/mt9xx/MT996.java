...
/**
 * <strong>MT 996 - Answers</strong>
 *
 * <p>
 * SWIFT MT996 (ISO 15022) message structure:
 * <br>
 <div class="scheme"><ul>
...
public class MT996 extends AbstractMT implements Serializable {
	...
	private static final transient java.util.logging.Logger log = java.util.logging.Logger.getLogger(MT996.class.getName());
...
// end qualifiers constants	
	 ...
	 * @param m swift message with the MT996 content, the parameter can not be null
	 * @see #MT996(String)
	 */
	 ...
	 * @since 7.7
	 */
	public static MT996 parse(MtSwiftMessage m) {
		if (m == null) {
			return null;
		}
		return new MT996(m);
	 ...
	 * @since 7.7
	 */
	public MT996(final String sender, final String receiver) {
    ...
    * @param sender the sender address as a bic8, bic11 or full logical terminal consisting of 12 characters
	* @param receiver the receiver address as a bic8, bic11 or full logical terminal consisting of 12 characters
	* @see #MT996(String, String)
	* @deprecated Use instead <code>new MT996(sender, receiver)</code> instead
	*/
	@Deprecated
	...
	public MT996(final int messageType, final String sender, final String receiver) {
		super(996, sender, receiver);
		com.financial.deprecation.DeprecationUtils.phase3(getClass(), "MT996(int, String, String)", "Use the constructor MT996(sender, receiver) instead.");
	}
	
	/**
	 * Creates a new MT996 by parsing a String with the message content in its swift FIN format.<br>
    ...
    private void sanityCheck(final SwiftMessage param) {
    	if (param.isServiceMessage()) {
			...
			log.warning("Creating an MT996 object from FIN content with message type "+param.getType());
		}
    }
	
	/**
	 * Creates a new MT996 by parsing a String with the message content in its swift FIN format.<br>
	 * If the fin parameter cannot be parsed, the returned MT996 will have its internal message object
	 * initialized (blocks will be created) but empty.<br>
	 * If the string contains multiple messages, only the first one will be parsed. 
	 *
	 * @param fin a string with the MT message in its FIN swift format. <em>fin may be null in which case this method returns null</em>
	 * @return a new instance of MT996 or null if fin is null 
	 * @since 7.7
	 */
	public static MT996 parse(final String fin) {
		if (fin == null) {
			return null;
		}
		return new MT996(fin);
    }
    
    /**
	 * Creates a new MT996 by parsing a input stream with the message content in its swift FIN format, using "UTF-8" as encoding.<br>
	 ...
	 * @since 7.7
	 */
	public MT996(final InputStream stream) throws IOException {
	 ...
	 *
	 * @param stream an input stream in UTF-8 encoding with the MT message in its FIN swift format.
	 * @return a new instance of MT996 or null if stream is null or the message cannot be parsed 
	 * @throws IOException if the stream data cannot be read
	 * @since 7.7
	 */
	public static MT996 parse(final InputStream stream) throws IOException {
		if (stream == null) {
			return null;
		}
		return new MT996(stream);
    }
    
    /**
	 * Creates a new MT996 by parsing a file with the message content in its swift FIN format.<br>
	 ...
	 * @since 7.7
	 */
	public MT996(final File file) throws IOException {
	 ...
	 *
	 * @param file a file with the MT message in its FIN swift format.
	 * @return a new instance of MT996 or null if; file is null, does not exist, can't be read, is not a file or the message cannot be parsed
	 * @throws IOException if the file content cannot be read
	 * @since 7.7
	 */
	public static MT996 parse(final File file) throws IOException {
		if (file == null) {
			return null;
		}
    ...
    /**
	 * Creates an MT996 messages from its JSON representation.
	 * <p>
	 * For generic conversion of JSON into the corresopnding MT instance
	 * see {@link AbstractMT#fromJson(String)}
	 *
	 * @param json a JSON representation of an MT996 message
	 * @return a new instance of MT996
	 * @since 7.10.3
	 */
	public final static MT996 fromJson(String json) {
		return (MT996) AbstractMT.fromJson(json);
	}

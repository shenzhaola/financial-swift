
/*
 * Copyright 2006-2018 Prowide
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.financial.swift;

import com.financial.Generated;


/**
 * Constants for qualifiers found in MTs starting with "F".
 * <em>WARNING: These constants are automatically generated from SRU information
 * they may change without entering the formal Prowide Deprecation Policy
 * http://www.singlee.financial.com/resources/deprecation-policy</em>
 * @since 7.8.1
 * 
 * 
 */
@Generated
public interface SchemeConstantsF {

    public final static String FF = "FF";
    public final static String FC = "FC";
    public final static String F = "F";
    public final static String FBF = "FBF";
    public final static String FAXT = "FAXT";
    public final static String FEOMA = "FEOMA";
    public final static String FLIPPED = "FLIPPED";
    public final static String FLOATFIXED = "FLOATFIXED";
    public final static String FRABBA = "FRABBA";
    public final static String FIXEDFLOAT = "FIXEDFLOAT";
    public final static String FLOORBUYER = "FLOORBUYER";
    public final static String FRN = "FRN";
    public final static String FLOORSLLER = "FLOORSLLER";
    public final static String FIXEDFIXED = "FIXEDFIXED";
    public final static String FLOATFLOAT = "FLOATFLOAT";
    public final static String FOLLOWING = "FOLLOWING";
    public final static String FLAT = "FLAT";
    public final static String FIRST = "FIRST";
    public final static String FXORDER = "FXORDER";
    public final static String FXTR = "FXTR";
    public final static String FXOR = "FXOR";
    public final static String FROZ = "FROZ";
    public final static String FRAU = "FRAU";
    public final static String FRWD = "FRWD";
    public final static String FIAC = "FIAC";
    public final static String FIAN = "FIAN";
    public final static String FRNF = "FRNF";
    public final static String FRNR = "FRNR";
    public final static String FCOU = "FCOU";
    public final static String FIA = "FIA";
    public final static String FORM = "FORM";
    public final static String FICL = "FICL";
    public final static String FXIB = "FXIB";
    public final static String FXIS = "FXIS";
    public final static String FORF = "FORF";
    public final static String FROM = "FROM";
    public final static String FORC = "FORC";
    public final static String FMT = "FMT";
    public final static String FSBN = "FSBN";
    public final static String FSSA = "FSSA";
    public final static String FIN = "FIN";
    public final static String FIOP = "FIOP";
    public final static String FXIN = "FXIN";
    public final static String FXCX = "FXCX";
    public final static String FOLQ = "FOLQ";
    public final static String FLFR = "FLFR";
    public final static String FISC = "FISC";
    public final static String FXDT = "FXDT";
    public final static String FDAT = "FDAT";
    public final static String FTCA = "FTCA";
    public final static String FILL = "FILL";
    public final static String FRAQ = "FRAQ";
    public final static String FREEASS = "FREEASS";
    public final static String FOB = "FOB";
    public final static String FCA = "FCA";
    public final static String FOZ = "FOZ";
    public final static String FRFR = "FRFR";
    public final static String FAS = "FAS";
    public final static String FACT = "FACT";
    public final static String FRAUDMSG = "FRAUDMSG";

}

 ...
 */
package com.financial.swift.io.parser;

import com.financial.ProwideException;
import com.financial.deprecation.DeprecationUtils;
...
/**
 * FIN Parser. This implementation now properly supports all system messages (i.e: messages for MT 0xx) and service
 * messages (for example: ACK).<br>
 ...
 * </ul>
 * <br>
 * Field32A This is based in the old SwiftParser2, that is now deprecated.<br>
 *
...
public class SwiftParser {
    ...
    /**
     * Reference to the current message being parsed. This should be used when some parsing decision needs to be made
     * based on a previous item parsed, like a value in a previous tag or block.
     */
     ...
     *
     * The implementation uses the default parser behaviour which is lenient and will do a best effort to read as much
     * from the message content as possible regardless of the content and block boundaries beeing valid or not. For
     * instance, it will read the headers even if the value length is incorrect, and it will read the text block (block
     * 4) even if it is missing the closing hyphen and bracket. For more options check
     * {@link #setConfiguration(SwiftParserConfiguration)}
     *
    ...
    /**
     * @deprecated use {@link #consumeBlock(UnparsedTextList)} instead of this, <code>consumeBlock(null)</code> is
     *             acceptable
     */
    @Deprecated
    @ProwideDeprecated(phase4 = TargetYear._2019)
    protected SwiftBlock consumeBlock() throws IOException {
        DeprecationUtils.phase3(getClass(), "consumeBlock()",
            "Use consumeBlock(UnparsedTextList) instead of this, consumeBlock(null) is acceptable.");
        return consumeBlock(null);
    }

    /**
     * Consume the next block of the message on the reader. This methods seeks to a block start, then identifies the
     * block and calls the proper method to consume the block type that is coming, not all blocks are parsed in the same
     * manner.
     * 
     * @param unparsedReceiver may be null, the unparsedTextList that will receive the chunks that can not be identified
     *        sas part of the message
     *
    ...
    /**
     * Attempt to detect if block 2 refers to an input or output message. If the parameter block content is not
     * well-formed will return false as default.
     * 
    ...
    /**
     * Parses a block 4 from an input string. This method supports the two possible formats of a swift block 4:<br>
     * <ul>
     * <li><b>Text mode</b>: this is the common block 4 for categories 1 to 9.</li>
     * <li><b>Tag mode</b>: this is the same format as for blocks 3 and 5. This format is used by service messages (for
     * example: ACK) and system messages (category 0).</li>
     * </ul>
     * <br>
     *
     * @param b the block to set up tags into
     ...
     * <li>"}"</li>
     * </ul>
     * where "[LBR]" stands for any of: "[CR]", "[LF]" or "[CR][LF]" and "[X]" is any character other than [CR] and
     * [LF].<br>
     ...
     * The function search the string looking for the occurrence of the sequence "}". It is important to note that curly
     * braces are balanced along the search.
     * 
     * @param s the FIN input text
     * @param start the position to start analysis at
     ...
     * The received buffer contains only the pertinent data for the tag (name and value). Trailing [CR][LF] on the text
     * <b>MUST</b> not be present.
     *
     * @param buffer the buffer containing the tag
     * @param unparsedText the unparsed text to assign (use null if none is wanted). This single text is fragmented in
     *        multiple texts if there are more than one message.
     * @return a swift Tag
     * @throws IOException
     */
    ...
    /**
     * this method receives a string that is a sequence of unparsed text and splits it into different unparsed texts.
     * The algorithm is to split on message begin (i.e: "{1:" and balance curly braces). This last thing ensures that a
     * single message with unparsed text inner messages is treated as one single unparsed text.<br>
     * That is:<br>
     *
     * <pre>
     * {1:...}                 -- message block 1
    ...
    /**
     * Reads the buffer until end of block is reached. The initial character of the block must be already consumed and
     * the reader has to be ready to consume the first character inside the block
     *
     * <p>
     * This method assumes that the starting block character was consumed because that is required in order to identify
     * the start of a block, and call this method which reads until this block ends.
     *
     ...
     * <p>
     * Will return either a {@link SwiftBlock2Input} or {@link SwiftBlock2Output} depending on the parameter block
     * content.
     *
     ...
     * <p>
     * If you don't know the container message direction, user {@link #parseBlock2(String)} instead.
     *
